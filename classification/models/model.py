...
def split_up_model(model, arch_name, dataset_name):
    ...
    elif arch_name == "Kireev2021Effectiveness_RLATAugMix":
        normalization = ImageNormalizer(mean=model.mu, std=model.sigma)
        encoder = nn.Sequential(normalization, *list(model.children())[:-1], nn.AvgPool2d(kernel_size=4, stride=4), nn.Flatten())
